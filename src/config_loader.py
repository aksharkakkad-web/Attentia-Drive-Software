...
class FrameSourceConfig:
    ...
    threaded: bool = False    # Enable background capture thread (webcam only)
...
class ClassifierConfig:
    ...
    frame_skip: int = 1       # 1 = every frame (legacy). Set to 2 for Pi 5.
    num_threads: int = 1      # TFLite interpreter threads. Set to 4 for Pi 5.
...
class TelemetryConfig:
    ...
    flush_interval: int = 30  # Flush CSV to disk every N rows
...
class FaceDetectorConfig:
    ...
    inference_width: int = 0    # 0 = use original frame width (no resize)
    inference_height: int = 0   # 0 = use original frame height (no resize)
...
def _build_frame_source_config(data: dict) -> FrameSourceConfig:
    ...
    config.threaded = bool(data.get("threaded", config.threaded))
...
def _build_classifier_config(data: dict) -> ClassifierConfig:
    ...
    config.num_threads = int(data.get("num_threads", config.num_threads))
...
def _build_telemetry_config(data: dict) -> TelemetryConfig:
    ...
    config.flush_interval = int(data.get("flush_interval", config.flush_interval))
...
def _build_face_detector_config(data: dict) -> FaceDetectorConfig:
    ...
    config.inference_width = int(data.get("inference_width", config.inference_width))
    config.inference_height = int(data.get("inference_height", config.inference_height))
